...
use crate::{
    ...
    range_scan::{ScanIter, ScanIterMut},
...
impl BfTree {
    ...
    pub(crate) fn create_mem_page_size_classes(
        ...
        let mut size_class = base.pow(x) as usize * c + std::mem::size_of::<LeafNode>();
        ...
        while size_class <= max_mini_page_size {
            ...
            x += 1;
            size_class = base.pow(x) as usize * c + std::mem::size_of::<LeafNode>();
        ...
        if x == 0 || mem_page_size_classes[(x - 1) as usize] < max_mini_page_size {
            mem_page_size_classes.push(max_mini_page_size);
            x += 1;
        }

        // The largest page size is the full leaf page size
        if x == 0 || mem_page_size_classes[(x - 1) as usize] < leaf_page_size_in_byte {
    ...
    pub fn with_config(config: Config, buffer_ptr: Option<*mut u8>) -> Self {
        ...
        if config.cache_only {
            assert!(
                config.cb_size_byte >= 4 * config.leaf_page_size,
                "In cache-only mode, cb_size_byte should be at least 4 times of leaf_page_size"
            );
        } else {
            assert!(
                config.cb_size_byte >= 2 * config.leaf_page_size,
    ...
    pub fn delete(&self, key: &[u8]) {
        // The input key cannot exceed the configured max key length
        if key.len() > self.config.max_fence_len / 2 {
            return;
        }
