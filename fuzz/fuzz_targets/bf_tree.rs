...
use bf_tree::{BfTree, Config, LeafReadResult};
...
fuzz_target!(|methods: Vec<Methods>| {
    ...
    let bf_tree = BfTree::with_config(config.clone(), None);
    ...
    for m in methods {
        match m {
            ...
            Methods::Scan((s, len)) => {
                ...
                let mut bf_v = bf_tree.scan(&k, len as usize);
                ...
                for v in std_v {
                    let bf_len = bf_v.next(&mut buffer).unwrap();
                    assert_eq!(v[..], buffer[0..bf_len as usize])
                }
    ...
    bf_tree.snapshot();
    drop(bf_tree);
    let bf_tree = BfTree::new_from_snapshot(config.clone(), None);
